...
namespace eosio { namespace chain {
                                                                             ...
                                                                             )const
   {
      ...
      try {
         ...
         if( *linked_permission == config::remme_any_name )
            return optional<permission_name>();
                                                           ...
                                                           )const
   {
      ...
      if( !linked_permission_name ) // if action is linked to remme.any permission
         return;
                                                             ...
                                                             )const
   {
      ...
      if( *unlinked_permission_name == config::remme_any_name )
         return;
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is remme.any
