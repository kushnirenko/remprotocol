...
First, the account `remme.wrap` needs to be created. Since it has the restricted `remme.` prefix, only a privileged account can create this account. So this guide will use the `remme` account to create the `remme.wrap` account. On typical live blockchain configurations, the `remme` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `remme.msig` contract to help coordinate the approvals of the proposed transaction that creates the `remme.wrap` account.

The `remme.wrap` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`remme`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `remme` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `remme` account.
...
```
$ cleos system newaccount -s -j -d --transfer --stake "1.000 SYS" --buy-ram-kbytes 50 remme remme.wrap EOS8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
...
The actual public key used is not important since that data is only encoded into the `remme::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `remme::newaccount` action. It should look like:
```
...
Third, generate a transaction containing the actual `remme::newaccount` action that will be used in the final transaction:
```
$ cleos push action -s -j -d remme newaccount newaccount_payload.json -p remme > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `remme::setpriv` action which will make the `remme.wrap` account privileged:
```
$ cleos push action -s -j -d remme setpriv '{"account": "remme.wrap", "is_priv": 1}' -p remme > generated_setpriv_trx.json
...
Then generate the template for the transaction containing the `remme.wrap::exec` action:
```
$ cleos push action -s -j -d remme.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
$ cat wrap_update_alice_owner_trx.json
{
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodeos/cleos, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `remme.wrap::exec` action is available in the output of the `cleos multisig review` command, only the hex data is available of the payload of the inner `remme::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodeos/cleos, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `remme::setcode` action to set the contract code of the `remme.wrap` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
