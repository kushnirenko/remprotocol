...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.cleos + 'create account remme ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.cleos + 'system newaccount --transfer remme %s %s --stake "%s"   ' %
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu)))
        if unstaked:
            retry(args.cleos + 'transfer remme %s "%s"' % (a['name'], intToCurrency(unstaked)))
...
def claimRewards():
    table = getJsonOutput(args.cleos + 'get table remme remme producers -l 100')
    times = []
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'remme', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'remme', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.cleos + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'remme setcode' + jsonArg(setcode) + ' -p ' + proposer)
...
def stepStartWallet():
    ...
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'remme', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(1.5)
...
def stepCreateTokens():
    run(args.cleos + 'push action remme.token create \'["remme", "100000000.0000 %s"]\' -p remme.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.cleos + 'push action remme.token issue \'["remme", "%s", "memo"]\' -p remme' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.cleos + 'set contract remme ' + args.contracts_dir + '/remme.system/')
    sleep(1)
    run(args.cleos + 'push action remme setpriv' + jsonArg(['remme.msig', 1]) + '-p remme@active')
def stepInitSystemContract():
    run(args.cleos + 'push action remme init' + jsonArg(['0', '4,' + args.symbol]) + '-p remme@active')
    sleep(1)
