...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(remme.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(remme.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::remme_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(remme.token));
         set_code( N(remme.token), contracts::remme_token_wasm() );

         auto result = push_action(N(remme.token), N(create), mutable_variant_object()
                 ("issuer",       remme_token)
         ...
         result = push_action(N(remme.token), N(issue), mutable_variant_object()
                 ("to",       remme_token)
      ...
      static const std::string remme_token;
};

const std::string currency_tester::remme_token = name(N(remme.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(remme.token), expected.get_symbol(), N(remme.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(remme.token), N(transfer), mutable_variant_object()
         ("from", remme_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(remme.token), N(transfer), mutable_variant_object()
                                    ("from", remme_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(remme_token, N(alice), "100.0000 CUR");
   ...
   // issue to remme_token using right precision
   {
      auto trace = issue(N(remme.token), "25.0256 CUR");

      BOOST_CHECK_EQUAL(true, chain_has_transaction(trace->id));
      BOOST_CHECK_EQUAL(asset::from_string("999925.0256 CUR"), get_balance(N(remme.token)));
   }
