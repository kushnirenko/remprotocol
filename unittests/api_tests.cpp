...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( N(test), contracts::remme_bios_wasm() );
   set_abi( N(test), contracts::remme_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) { try {
...

   {
      ...
      auth.accounts.push_back( permission_level_weight{{N(testapi), config::remme_code_name}, 1} );
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@remme.code.
      dtt_action dtt_act3;
