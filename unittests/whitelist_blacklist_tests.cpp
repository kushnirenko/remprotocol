...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(remme.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(remme.token), contracts::remme_token_wasm() );
         chain->set_abi(N(remme.token), contracts::remme_token_abi().data() );
         chain->push_action( N(remme.token), N(create), N(remme.token), mvo()
              ( "issuer", "remme.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(remme.token), N(issue), N(remme.token), mvo()
              ( "to", "remme.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(remme.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(remme.token), N(alice)};
   test.init();

   test.transfer( N(remme.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(remme.token), N(bob)};
   test.init();

   test.transfer( N(remme.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(remme.token) );
   ...
   test.chain->set_code(N(bob), contracts::remme_token_wasm() );
   test.chain->set_abi(N(bob), contracts::remme_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::remme_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::remme_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(remme.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_eosio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, contracts::remme_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
